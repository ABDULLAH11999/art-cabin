...
function getPageMeta(pathname: string | null) {
  if (pathname?.startsWith("/admin/arts/new")) return { title: "Create Art", crumb: "Arts / Create" };
  if (pathname?.startsWith("/admin/arts/")) return { title: "Edit Art", crumb: "Arts / Edit" };
  if (pathname?.startsWith("/admin/arts")) return { title: "Arts", crumb: "Catalog / Arts" };
    ...
    <div className="min-h-screen w-full overflow-x-hidden bg-white text-black">
      <div className="lg:grid lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="hidden min-h-screen border-r border-black/10 bg-white lg:flex lg:flex-col">
          <div className="border-b border-black/10 px-6 py-5">
            <Link href="/admin" className="flex items-center gap-3">
              ...
              <span>
                <span className="block font-heading text-xl font-semibold leading-none">Art Cabin</span>
                ...
                <Link
                  ...
                  className={`flex items-center justify-between rounded-2xl border px-4 py-3.5 text-sm font-medium transition ${
                    active
                      ...
                      : "border-black/10 bg-white text-black hover:border-maroon hover:bg-maroon hover:text-white"
                  }`}
                >
        ...
        <div className="min-w-0 overflow-x-hidden">
            ...
            <div className="flex items-center justify-between gap-3 px-3 py-3 sm:px-4 lg:px-6 lg:py-4">
              <div className="flex min-w-0 items-center gap-2 sm:gap-3">
                <Link href="/admin" className="flex items-center gap-3 lg:hidden">
                  <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-maroon text-white shadow-luxe">
                    <Sparkles className="h-4 w-4" />
                  </div>
                  <span className="min-w-0">
                    <span className="block truncate font-heading text-base font-semibold leading-none sm:text-lg">Art Cabin</span>
              ...
              <div className="hidden items-center gap-3 lg:flex">
                <Link
                  href="/"
                  target="_blank"
                  className="rounded-2xl border border-maroon px-4 py-3 text-sm font-semibold text-maroon transition hover:bg-maroon hover:text-white"
                >
                  View Website
                </Link>
                <button
                  onClick={() => signOut({ callbackUrl: "/admin/login" })}
                  className="rounded-2xl bg-maroon px-4 py-3 text-sm font-semibold text-white transition hover:bg-maroonSoft"
                >
                  Logout
                </button>
              </div>
              ...
              <div className="flex items-center gap-2 lg:hidden">
                ...
                <button
                  onClick={() => signOut({ callbackUrl: "/admin/login" })}
                  className="rounded-2xl bg-maroon px-3 py-2 text-[11px] font-semibold text-white"
                >
                  Logout
                </button>
              </div>
          ...
          <main className="overflow-x-hidden px-3 py-4 pb-28 sm:px-4 lg:px-6 lg:py-6 lg:pb-6">{children}</main>
        </div>
      </div>
            ...
            return (
              <Link
                ...
                className={`min-w-0 flex flex-col items-center justify-center rounded-2xl px-1 py-2 text-[9px] font-semibold transition ${
                  active ? "bg-maroon text-white shadow-sm" : "text-black/55"
                }`}
              >
                <Icon className="h-4 w-4" />
