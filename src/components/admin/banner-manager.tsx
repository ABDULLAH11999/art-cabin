...
function BannerColumn({
  ...
  return (
    <section className="rounded-3xl border border-maroon/15 bg-white p-4 shadow-sm sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="font-heading text-2xl text-maroon">{title}</h2>
          <p className="mt-1 text-sm text-black/55">{description}</p>
        </div>
        <button type="button" onClick={onAdd} className="rounded-2xl bg-maroon px-4 py-3 text-sm font-semibold text-white">
          Add Images
        </button>
      </div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {images.map((image) => (
          <div key={image} className="overflow-hidden rounded-3xl border border-maroon/15 bg-maroon/5">
            ...
            <div className="flex items-center justify-between gap-3 p-3">
              ...
              <button
                ...
                className="rounded-full border border-maroon/20 px-3 py-1 text-xs font-semibold text-maroon"
              >
                Remove
              </button>
            </div>
        ...
        {images.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-maroon/20 p-8 text-center text-sm text-black/45">
...
export function BannerManager({ initialSettings }: { initialSettings: BannerSettings }) {
  const [desktopImages, setDesktopImages] = useState<string[]>(initialSettings.desktopImages);
  const [mobileImages, setMobileImages] = useState<string[]>(initialSettings.mobileImages);
  const [saving, setSaving] = useState(false);
  const desktopInputRef = useRef<HTMLInputElement | null>(null);
  const mobileInputRef = useRef<HTMLInputElement | null>(null);
  const { uploadFiles, uploading } = useMediaUploader("art-cabin/banners");
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 font-heading text-3xl md:text-5xl">Desktop and Mobile Banners</h1>
      </div>

      <div className="grid gap-6 xl:grid-cols-2">
        <BannerColumn
          title="Desktop Banner"
          description="Add one or more images for the desktop hero slider."
        ...
        <BannerColumn
          title="Mobile Banner"
          description="Add one or more images for the mobile hero slider."
          images={mobileImages}
          uploading={uploading}
