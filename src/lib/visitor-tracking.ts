export type VisitorPlatform = "INSTAGRAM" | "GOOGLE_SEARCH" | "SHARED_LINK";
...
function hasInstagramSignals(referrer: string, url: string, userAgent: string) {
  return (
    referrer.includes("instagram") ||
    url.includes("igshid=") ||
    url.includes("utm_source=instagram") ||
    url.includes("utm_campaign=instagram") ||
...
export function classifyVisitorPlatform(input?: string | VisitorSource | null) {
  ...
  if (hasInstagramSignals(referrer, url, userAgent)) {
    return "INSTAGRAM" satisfies VisitorPlatform;
  }
